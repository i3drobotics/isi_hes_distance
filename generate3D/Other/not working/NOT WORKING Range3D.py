...
def main(number):
    ...
    try:
        ...
        #load camera images
        left_fns = glob.glob(args.input_folder + '/{}'.format(number) + args.left_wildcard)
        right_fns = glob.glob(args.input_folder + '/{}'.format(number) + args.right_wildcard)
        pose_fns = glob.glob(args.input_folder + '/{}'.format(number) + args.pose_wildcard)
        print(glob.glob(args.input_folder + '/{}'.format(number) + args.left_wildcard))
        ...
        while i < len(left_fns):
            ...
            left_fn_basename = os.path.splitext(os.path.basename(left_fn))[0] + number
...
def Distance(trials,number):
    #opens the point cloud and converts it to floats.
    global left_fn_basename
    global combi_dist
    
    print('opening point cloud...')
    file = 'output/point_clouds/'+left_fn_basename+'_point_cloud.ply'
    ...
    print('generating shortest distances...')
    close_pos = []
    ...
    """
    str_av_dist = str(average_dist)
    print('writing distance.txt...')
    f2 = open("output/" + left_fn_basename + "_Distance.txt","w+")
    f2.write(str_av_dist)
    f2.close()
    print('finished; distance can be found in output/'+left_fn_basename+'_distance.txt')
    """

    combi_dist += average_dist
