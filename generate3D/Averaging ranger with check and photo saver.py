...
while True:
    ...
    if count > 0 and count <= number:
        ...
        if count == 1:
            print('Saving image in Example Images...')
            cv2.imwrite('Example Images/image_'+str(images)+'_l_.png',Ldst)
            cv2.imwrite('Example Images/image_'+str(images)+'_r_.png',Rdst)
            print(Ldst)
    ...
    if count > number:
        ...
        #display and save the results for the brightest area
        cv2.imwrite('Example Images/blur_'+str(images)+'.png',image)
        ...
        if abs(av_pix_dist - x_dist) > 0.1 or abs(av_pix_dist - y_dist) > 0.05:
            print("Distance can't be determined;")
            print("Target could be beyond 1.5m,")
            print("Probe could be unalined,")
            print("object could be slanted")
